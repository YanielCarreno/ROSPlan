#include "rosplan_action_interface/MRPActionInterface.h"

/* The implementation of RPMoveBase.h */
namespace KCL_rosplan {

	/* run action interface */
	void MRPActionInterface::runMRPActionInterface() {
		...
		// set robot name
		nh.getParam("r_name", robot.r_name);
		...
		if(client.call(srv)) {
			...
			ROS_ERROR("KCL: (MRPActionInterface) could not call Knowledge Base for operator details, %s", params.name.c_str());
			return;
		}
		...
		for(; nit!=predicateNames.end(); nit++) {
			...
			if(predClient.call(predSrv)) {
				...
				ROS_ERROR("KCL: (MRPActionInterface) could not call Knowledge Base for predicate details, %s", params.name.c_str());
				return;
			}
		}
    ...
    ops.template init<rosplan_dispatch_msgs::ActionDispatch>(adt, 1000, boost::bind(&KCL_rosplan::MRPActionInterface::dispatchCallback, this, _1));
    ops.transport_hints = ros::TransportHints();
    ops.allow_concurrent_callbacks = true;
		ros::Subscriber ds = nh.subscribe(ops); // nh.subscribe(adt, 1000, &KCL_rosplan::MRPActionInterface::dispatchCallback, this);
